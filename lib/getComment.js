...
module.exports.getComment = async (bookmark, ignores) => {
  ...
  for (const c of comments) {
    ...
    try {
      ...
      // NOTICE: comment_date が bookmark.updated_at より古い場合は、
      // 既に登録済みのコメントと判断してスキップする処理だった。
      // ブックマークの初期登録時に updated_at がセットされるようになったため、
      // この処理をコメントアウトし、コメントが post されているかどうかは、
      // bookmark.users に含まれているかどうかで判断するようにした。
